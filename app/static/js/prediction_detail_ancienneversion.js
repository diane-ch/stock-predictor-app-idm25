...
// ===== NEW: Slider visibility control =====
function toggleSliderVisibility(period) {
  ...
  // Hide slider for 1M and 1Y periods, show for 1W
  if (period === '1M' || period === '1Y') {
    slider.style.display = 'none';
    if (vGuide) {
      vGuide.style.display = 'none';
    }
    console.log(`🔒 Slider hidden for period: ${period}`);
  } else {
    slider.style.display = 'block';
    if (vGuide) {
      vGuide.style.display = 'block';
    }
    console.log(`🔓 Slider shown for period: ${period}`);
  }
}
...
function setRangeWithComparisonData(period, ticker) {
  ...
  // Control slider visibility based on period
  toggleSliderVisibility(period);
